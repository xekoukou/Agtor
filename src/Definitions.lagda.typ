...
>>Tₘ : ∀{𝓠 𝓦} → {A : 𝓠 ̇ } → (B : A → 𝓦 ̇ ) → (C : 𝓦 ̇ ) → A + 𝟙 {𝓤₀} → 𝓦 ̇
>>Tₘ B C (inl x) = B x
>>Tₘ B C (inr _) = C

_>>ₘ_∣_ : ∀{𝓠 𝓦} → {A : 𝓠 ̇ } → {B : A → 𝓦 ̇ } → {C : 𝓦 ̇ } → (x : A + 𝟙 {𝓤₀}) → C → ((x : A) → B x) → >>Tₘ B C x
inl x >>ₘ c ∣ f = f x
inr _ >>ₘ c ∣ f = c

_>>ₘ'_∣_ : ∀{𝓠 𝓦} → {A : 𝓠 ̇ } → {B : 𝓦 ̇ } → {C : 𝓦 ̇ } → (x : A + 𝟙 {𝓤₀}) → C → ((x : A) → B) → >>Tₘ (λ _ → B) C x
x >>ₘ' c ∣ f = x >>ₘ c ∣ f

_>>ₘ''_∣_ : ∀{𝓠 𝓦} → {A : 𝓠 ̇ } → {B : 𝓦 ̇ } → (x : A + 𝟙 {𝓤₀}) → B → ((x : A) → B) → B
inl x >>ₘ'' c ∣ f = f x
inr x >>ₘ'' c ∣ f = c
...
_＝₂_ : ∀ {𝓥} → (x y : BSet 𝓥 × 𝟚) → 𝓤 ⊔ 𝓥 ̇
x ＝₂ y = (x ₂ ＝ y ₂) × (< < y > > ⇔ₚ < < x > >)


<&PSet> : ∀ 𝓥 𝓦 → 𝓤 ⊔ 𝓥 ⁺ ⊔ 𝓦 ⁺ ̇
<&PSet> 𝓥 𝓦 = Pred (BSet 𝓥 × 𝟚) 𝓦 
