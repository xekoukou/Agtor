...
record OOpTerm (n : ℕ) : Type (ℓ-max ℓ (ℓ-suc ℓ')) where
  constructor FSt
  field 
    ⟨_⟩ : Type ℓ'
    fs : ⟨_⟩ → C n
...
data FRel {n : ℕ} (fq1 : OOpTerm n) : OOpTerm n → Type (ℓ-max ℓ (ℓ-suc ℓ')) where
  frel : {A : Type ℓ'} {f : A → ⟨ fq1 ⟩} → {g : ⟨ fq1 ⟩ → A} → retract f g → FRel fq1 (FSt A (fq1 .fs ∘ f))

open FRel

OpTerm : ℕ → Type (ℓ-max ℓ (ℓ-suc ℓ'))
OpTerm n = OOpTerm n / FRel {n}

dd : ∀{n : ℕ} → {ffst1 ffst2 : OOpTerm n} → (v1 : ⟨ ffst1 ⟩) → (r : FRel ffst1 ffst2) → ⟨ ffst2 ⟩
dd {_} {ffst1} v1 (frel {_} {f} {g} x) = g v1


data FDom {n : ℕ} : OpTerm n → Type (ℓ-max ℓ (ℓ-suc ℓ')) where
  vl : {ffst : OOpTerm n} →  ⟨ ffst ⟩ → FDom [ ffst ]
  eqd : {ffst1 : OOpTerm n} → {A : Type ℓ'} → (v2 : A) → {f : A → ⟨ ffst1 ⟩} → {g : ⟨ ffst1 ⟩ → A} → (rt : retract f g)
        → PathP (λ i → FDom (eq/ ffst1 (FSt A (ffst1 .fs ∘ f)) (frel {A = A} {f} {g} rt) i)) (vl (f v2)) (vl v2)
  sqd : ∀{ x y : OpTerm n} → (p1 p2 : x ≡ y) → (dx : FDom x) → (dy : FDom y)
       → (dp1 : PathP (λ i → FDom (p1 i)) dx dy)
       → (dp2 : PathP (λ i → FDom (p2 i)) dx dy) → SquareP (λ i j → FDom (squash/ x y p1 p2 i j)) dp1 dp2 refl refl


_$_ : {n : ℕ} → (os : OpTerm n) → FDom os → C n
