...
module _ {ℓ ℓ' : _} (prM : UMTypePr ℓ ℓ') where
  ...
  mutual
    ...
    record PrimActor : Type (ℓ-suc (ℓ-max ℓ ℓ')) where
      inductive
      field
        P     : UMType → Type
        decP  : ∀ A → Dec (P A)
...
-- This has no meaning, it is a variable with type ActorT
    data Actor : Type (ℓ-suc (ℓ-max ℓ ℓ')) where
      poly : PolyV → Actor
      msg  : ValT → Actor
      primActor : PrimActor → Actor
